...
return apply_filters( 'wc_cardknox_apple_settings',
	array(
		...
		'applepay_capture' => array(
			...
			'type'        => 'checkbox',
			'id'       	  => 'cardknox_capture',
			'description' => __( 'Whether or not to capture the transaction immediately for Apple Pay. If unchecked, the transaction will require later capturing.', 'woocommerce-gateway-cardknox' ),
			'default'     => 'yes',
			'desc_tip'    => true,
		),
		'applepay_auth_only_order_status' => array(
			...
			'description' => __( 'Specifies the desired order status after an authorization-only transaction for Apple Pay.', 'woocommerce-gateway-cardknox' ),
			'default'     => 'on-hold',
			'desc_tip'    => true,
			'options'  => array(
