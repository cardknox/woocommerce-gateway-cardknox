...
jQuery(document.body).on("updated_cart_totals", function () {
  ...
  const apRequest = {
    ...
    authorize: function (applePayload, totalAmount) {
      ...
      if (appToken) {
        ...
        let billingAddress = applePayload.billingContact.addressLines.filter(function(line) {
        ...
        shippingTelephone = shippingTelephone.substring(shippingTelephone.indexOf(" ") + 1);

        let shippingAddress = applePayload.shippingContact.addressLines.filter(function(line) {
            return line; // This will remove any falsy values: undefined, null, "", 0, false, NaN
        }).join(" ");
        ...
        applePaycreateWooCommerceOrder(xcardnum, totalAmount, billingContact, shippingContact, lastSelectedShippingMethod);
  ...
  function applePaycreateWooCommerceOrder(token, amount, billingContact, shippingContact, lastSelectedShippingMethod) {
    jQuery.ajax({
        url: applePaysettings.ajax_url,
        type: 'POST',
        data: {
        ...
        success: function (response) {            
            if (response.success) {
                ...
                jQuery(".applepay-error").html("<div> " + response.data + " </div>").show();
                setTimeout(function () {
        ...
        error: function (error) {
            ...
            jQuery(".applepay-error").html("<div> " + error.responseText + " </div>").show();
            setTimeout(function () {
                jQuery(".applepay-error").html("").hide();
            }, 3000);
        }
