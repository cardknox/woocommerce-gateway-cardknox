jQuery(document.body).on('updated_checkout', function() {
    if (applePaysettings.merchant_identifier == "" || applePaysettings.merchant_identifier == null || applePaysettings.merchant_identifier.length == 0) {
        jQuery(".applepay-error").html("<div class='woocommerce-error' role='alert'>Please contact support. Failed to initialize Apple Pay. </div>").show();
    } else {
...
window.apRequest = {
    ...
    apButtonLoaded: function(resp) {
        ...
        if (resp.status === iStatus.success) {            
            ...
            jQuery(".applepay-error").html("<div class='woocommerce-error'>"+resp.reason+"</div>").show();
