...
class WCCardknoxApplepay extends WC_Payment_Gateway_CC
{
    ...
    public function custom_admin_upload_btn_css($hook)
    {
        ...
        $custom_css = '
            ...
            .woocommerce table.form-table td small{
                margin-top:15px;
                display:block;
            }
        ';
    ...
    public function process_admin_options()
    {
        ...
        if (
            ...
            ! isset($_FILES[$field_key]['error']) ||
            UPLOAD_ERR_OK !== (int) $_FILES[$field_key]['error']
        ) {
            return;
        }
        ...
        // Root of WP install: ABSPATH -> /public_html/site/
        // dirname(dirname(ABSPATH)) -> /public_html/
        $target_dir = untrailingslashit(dirname(dirname(ABSPATH))) . '/.well-known';
        ...
        /**
         * Permissions: avoid hardcoding 0755 (SonarCloud warning).
         * Use WP's filesystem constants when available.
         */
        $desired_dir_perms = defined('FS_CHMOD_DIR') ? FS_CHMOD_DIR : 0755;
        $current_dir_perms = fileperms($target_dir) & 0777;

        // Only attempt change if different.
        if ($current_dir_perms !== $desired_dir_perms) {

            $fs_ready = function_exists('WP_Filesystem') ? WP_Filesystem() : false;
            global $wp_filesystem;

            if ($fs_ready && $wp_filesystem && is_object($wp_filesystem)) {
                $wp_filesystem->chmod($target_dir, $desired_dir_perms);
            } else {
                chmod($target_dir, $desired_dir_perms);
            }
        }
        ...
        // Construct verification URL pointing to the root domain.
        ...
        // ---- WordPress HTTP API request (replaces cURL) ----
        $args = array(
            'timeout'     => 15,
            'redirection' => 5,
            'sslverify'   => true,
            'user-agent'  => 'ApplePayDomainVerification',
        );
...
<?php
    ...
    public function init_form_fields()
    {
        ...
        // Start the link URL functionality.
        $filename    = 'apple-developer-merchantid-domain-association';
        $target_dir  = untrailingslashit(dirname(dirname(ABSPATH))) . '/.well-known';
        $target_path = trailingslashit($target_dir) . $filename;

        // Build ROOT domain URL (no subfolder)
        $parsed_url = wp_parse_url(home_url());
        $scheme     = isset($parsed_url['scheme']) ? $parsed_url['scheme'] : 'https';
        $host       = isset($parsed_url['host']) ? $parsed_url['host'] : '';

        // Only accept EXACT filename
        $exists = (file_exists($target_path) && is_readable($target_path));

        $existing_url = '';

        // IMPORTANT: URL only if file exists
        if ($exists && ! empty($host)) {
            $existing_url = $scheme . '://' . $host . '/.well-known/' . $filename;

            // Keep DB in sync
            $this->update_option('applepay_certificate', esc_url_raw($existing_url));
        } else {
            // Clear stale option when file is missing
            $this->update_option('applepay_certificate', '');
        }

        // Optional: detect renamed/similar files and show warning
        $similar_files = glob(trailingslashit($target_dir) . '*' . $filename . '*');
        $has_similar   = (is_array($similar_files) && ! empty($similar_files));

        if (isset($this->form_fields['applepay_certificate'])) {

            $desc = isset($this->form_fields['applepay_certificate']['description'])
            ...
            $this->form_fields['applepay_certificate']['description'] = $desc;
            $this->form_fields['applepay_certificate']['desc_tip']    = false;
        }
    }
