    ...
    class WC_Cardknox
    {
        ...
        */
        ...
        public function quickChekoutPaymentScripts()
        {
            ...
            // Sirf cart page par hi quick checkout scripts chahiye
            if (! is_cart()) {
                return;
            }

            // Common iFields script
            $this->enqueue_ifields_script_for_quickcheckout();

            // Options ko helpers se normalize karo
            $gpay_options  = $this->get_googlepay_quickcheckout_settings();
            $apple_options = $this->get_applepay_quickcheckout_settings();

            // Google Pay on Cart
            if ('no' === $gpay_options['googlepay_quickcheckout']) {
                $this->enqueue_googlepay_quickcheckout_cart($gpay_options);
            }

            // Apple Pay on Cart
            if ('no' === $apple_options['applepay_quickcheckout']) {
                $this->enqueue_applepay_quickcheckout_cart($apple_options);
            }
        }

        /**
         * Common iFields script (pehle aap is_cart() ke andar kar rahe the)
         */
        private function enqueue_ifields_script_for_quickcheckout()
        {
            wp_enqueue_script(
                'cardknox',
        ...
        /**
         * Google Pay options normalize
         */
        private function get_googlepay_quickcheckout_settings()
        {
        ...
        /**
         * Apple Pay options normalize
         */
        private function get_applepay_quickcheckout_settings()
        {

            $options  = (array) get_option('woocommerce_cardknox-applepay_settings', []);
        ...
        /**
         * Google Pay – Cart quick checkout ke liye CSS/JS + localized settings
         */
        private function enqueue_googlepay_quickcheckout_cart(array $options)
        {
            ...
            // Shipping costs (Google format)
            $shippingCostsGoogle = $this->get_shipping_costs_for_googlepay();

            // Shipping methods listing
            $methods = $this->get_shipping_methods_for_googlepay();
        ...
        /**
         * Apple Pay – Cart quick checkout ke liye CSS/JS + localized settings
         */
        private function enqueue_applepay_quickcheckout_cart(array $apple_options)
        {
            ...
            // Shipping costs (Apple format)
            $shippingCostsApple = $this->get_shipping_costs_for_applepay();

            $cardknoxApplepaySettings = [
                'enabled'               => $apple_options['applepay_enabled'],
                'title'                 => $apple_options['applepay_title'],
                'merchant_identifier'   => $apple_options['applepay_merchant_identifier'],
                'environment'           => $apple_options['applepay_environment'],
                'button_style'          => $apple_options['applepay_button_style'],
                'button_type'           => $apple_options['applepay_button_type'],
                'payment_action'        => $apple_options['applepay_capture'],
                'applicable_countries'  => $apple_options['applepay_applicable_countries'],
                'specific_countries'    => (array) $apple_options['applepay_specific_countries'],
                'total'                 => (float) WC()->cart->total,
                'shippingMethods'       => $shippingCostsApple,
                'ajax_url'              => admin_url('admin-ajax.php'),
                'create_order_nonce'    => wp_create_nonce('create_order_nonce'),
        ...
        /**
         * Google Pay ke liye shipping cost array (pehle jaisa hi, sirf helper me)
         */
        private function get_shipping_costs_for_googlepay()
        {
            $shippingCostsGoogle = [];
            ...
            return $shippingCostsGoogle;
        }

        /**
         * Google Pay ke liye shipping methods meta
         */
        private function get_shipping_methods_for_googlepay()
        {
            $methods = [];
        ...
        /**
         * Apple Pay ke liye shipping costs (identifier/label/amount/detail)
         */
        private function get_shipping_costs_for_applepay()
        {
            $shippingCostsApple = [];
